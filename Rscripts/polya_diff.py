...
parser = argparse.ArgumentParser(description='Overview report of poly(A) tail lengths.')
parser.add_argument('-i', metavar='input', type=str, help="Input TSV.", required=True)

parser.add_argument('-g', metavar='global', type=str, help="Global results TSV.", required=True)

parser.add_argument('-t', metavar='per_tr', type=str, help="Per transcript results TSV.", required=True)

parser.add_argument('-c', metavar='min_cov', type=int, help="Minimum coverage.", required=True)

parser.add_argument('-r', metavar='report', type=str, help="Report PDF.", required=True)
...
def _filter_medians(mdf, min_cov, pages):
    total_tr = len(mdf)
    pos = (mdf['count']['control'] >= min_cov) & (mdf['count']['treatment'] > min_cov)
    pass_tr = sum(pos)
    mdf_flt = mdf[pos]
...
if __name__ == '__main__':
    ...
    mdf = tails[['contig', 'polya_length', 'group']].groupby(['contig', 'group']).agg(['median', 'count'])
    mdf = mdf.unstack(level=-1)
    mdf.columns = mdf.columns.droplevel()

    mdf_flt = _filter_medians(mdf, args.c, pages)
    mdf_flt.loc[:, ('median', 'diff')] = mdf_flt.loc[:, ('median', 'treatment')].values - mdf_flt.loc[:, ('median', 'control')].values
    mdf_flt = mdf_flt.sort_index(axis=1).copy()

    trs = mdf_flt.index.unique()

    tails_flt = tails[tails.contig.isin(trs)]
    _make_boxplot(tails[['contig', 'group', 'sample', 'polya_length']], by='group', title="Boxplot overview (no outliers).", pages=pages)
    
    _make_boxplot(tails_flt[['contig', 'group', 'sample', 'polya_length']], by='group', title="Boxplot overview after filtering (no outliers).", pages=pages)
    
    _make_pairplots(tails_flt[['contig', 'group', 'sample', 'polya_length']], title="Data overview after filtering.", pages=pages)
    ...
    title = "Global dist. (no outliers): p-value={:.3f} U={:.3f}".format(gp, gu)
    
    tails_flt_med = tails_flt[['group', 'polya_length']].groupby('group').median()

    _make_boxplot2(df=tails_flt, title=title, pages=pages, showfliers=False, mdf=tails_flt_med)
    ...
    for tr in trs:
        ...
        u, p = stats.mannwhitneyu(trd[trd.group == "control"].polya_length.values, trd[trd.group == "treatment"].polya_length.values, alternative="two-sided")
        ulist.append(u)
        plist.append(p)
    ...
    if args.x:
        for tr in mdf_flt.index:
            tr_data = tails_flt[['contig', 'group', 'polya_length']].set_index('contig').loc[tr]
            title = "{} (no outliers): FDR={:.3f} U={:.3f}".format(tr, mdf_flt.loc[tr]['FDR'].values[0], mdf_flt.loc[tr]['u'].values[0])
            _make_boxplot2(df=tr_data, title=title, pages=pages, showfliers=False, mdf=tr_data.groupby('group').median())
