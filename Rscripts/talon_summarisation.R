...
# main_outdir <- "/Users/stavris/Desktop/Projects/silvia_ont_umc/talon_analysis_reimplementation_3"
...
ggsave(file=paste(main_outdir, "general_stats_fig.png",sep="/"), width = 10, height = 6, units = "in", dpi = 1200)
...
rm(list=setdiff(ls(), c("filtered_table", "n", "outdir", "main_outdir", "input_groups")))
...
for(i in 1:length(samplegroup)) { 
  ...
  figure <- ggarrange(p1, ggarrange(p3, p2, ncol = 2, labels = c("B", "C")), nrow = 2, labels = "A") 
  ggsave(file=paste(main_outdir, paste(group_name, "general_stats_fig.png", sep="_"),sep="/"), width = 10, height = 6, units = "in", dpi = 1200)
